...
app.get('/api/os', (req, res) => { 
    db.query('SELECT * FROM ordens_servico ORDER BY id DESC', (err, rows) => res.json(rows)); 
});
...
app.post('/api/os', (req, res) => { 
    ...
    // Se selecionou uma peça, diminui 1 do estoque
    if (peca_id) {
        db.query('UPDATE estoque SET quantidade = quantidade - 1 WHERE id = ? AND quantidade > 0', [peca_id]);
    }

    db.query('INSERT INTO ordens_servico (modelo_veiculo, placa, descricao, peca_id, valor_mao_obra, valor_pecas, data_agendamento, horario_entrega, tempo_estimado, logistica, forma_pagamento, total_pago, status_servico) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)', 
    [modelo_veiculo, placa, descricao, peca_id || null, valor_mao_obra, valor_pecas, data_agendamento, horario_entrega, tempo_estimado, logistica, forma_pagamento, total_pago, 'Pendente'], 
    (err, result) => {
...
// Rota para marcar OS como Pronto
app.put('/api/os/:id/pronto', (req, res) => { 
    db.query('UPDATE ordens_servico SET status_servico = "Pronto" WHERE id = ?', [req.params.id], (err) => res.json({ msg: 'OK' })); 
});

app.delete('/api/os/:id', (req, res) => { 
    db.query('DELETE FROM ordens_servico WHERE id=?', [req.params.id], (err) => res.json({ msg: 'OK' })); 
});
